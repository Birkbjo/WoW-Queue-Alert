...
class Notifier {
    ...
    async init(argv) {
        ...
        if (!config.PUSHBULLET.DEVICE_ID || argv.setup) {
            await this.setup();
        } else {
            ...
            const answer = await inquirer.prompt([
                {
                    ...
                    name: 'device',
                    choices: res.devices.map(d => ({ name: `${d.nickname} [${d.type}]`, value: d })),
                },
            ]);
            this.device = answer.device.iden;
            config.PUSHBULLET.DEVICE_ID = this.device;
            fs.writeFileSync('config.json', JSON.stringify(config, null, 4).concat('\n'), 'utf8');
            log.info(`Device set to: ${answer.device.nickname} (id:${answer.device.iden})`);
        } catch (e) {
